...
public class GreedySearch extends SearchAlgorithm {
	...
	public int step(){
		...
		if( current.equals( goal ) ){
			...
			System.out.println( "Greedy first found goal in: " + steps + " steps" );
		...
		for(Hexagon<HexData> next : hexagonalGrid.getNeighborsOf( current ) ){
			...
			if( !closeList.contains(next) || newCost < nextData.getDistance() ){ 
				...
				int distanceToEnd = HexPrimary.hexagonalGridCalculator.calculateDistanceBetween( goal, next );
				nextData.setPriority( newCost + distanceToEnd );
